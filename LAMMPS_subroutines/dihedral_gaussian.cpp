...
DihedralGaussian::DihedralGaussian(LAMMPS *lmp) : Dihedral(lmp) {}

/* ---------------------------------------------------------------------- */

DihedralGaussian::~DihedralGaussian()
{
  if (allocated) {
...
void DihedralGaussian::compute(int eflag, int vflag)
{
...
void DihedralGaussian::allocate()
{
  allocated = 1;
...
void DihedralGaussian::coeff(int narg, char **arg)
{
  if (narg < 1 || narg > 2) error->all(FLERR,"Incorrect args for dihedral coefficients");
  if (!allocated) allocate();

  int ilo,ihi;
  force->bounds(FLERR,arg[0],atom->ndihedraltypes,ilo,ihi);

  double epsdihed_one = force->numeric(FLERR,arg[1]);
...
void DihedralGaussian::write_restart(FILE *fp)
{
...
void DihedralGaussian::read_restart(FILE *fp)
{
  allocate();
...
void DihedralGaussian::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ndihedraltypes; i++)
    fprintf(fp,"%d %g\n",i,epsdihed[i]);
}
